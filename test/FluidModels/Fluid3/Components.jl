...
Difference to Fluid1_Components: Connector with (Medium,m_flow,r,state) and Medium is also in state
...
###### Define state-dependent variables for plotting, depending on medium
const dependentVariables = Dict{AbstractString,Function}("p"   => ModiaMedia.pressure,
                                                         "T"   => ModiaMedia.temperature,
                                                         "d"   => ModiaMedia.density,
                                                         "h"   => ModiaMedia.specificEnthalpy,
                                                         "u"   => ModiaMedia.specificInternalEnergy,
                                                         "Cp"  => ModiaMedia.specificHeatCapacityCp,                                                     
                                                         "eta" => ModiaMedia.dynamicViscosity)

ModiaMath.variablesDependingOnStruct(state::ModiaMedia.SimpleMediumState)         = dependentVariables
ModiaMath.variablesDependingOnStruct(state::ModiaMedia.SimpleIdealGasMediumState) = dependentVariables
...
@model FixedSink begin
    ...
    @equations begin
        pressure(inPlug.state) + inPlug.r = p0
    end
end
...
@model Junction begin
    ...
    @equations begin
        ...
        pA = pressure(        inPlugA.state)
        pB = pressure(        inPlugB.state)
        hA = specificEnthalpy(inPlugA.state)
        hB = specificEnthalpy(inPlugB.state)
...
@equations begin
    ...
    pin  = pressure(        inPlug.state)
    hin  = specificEnthalpy(inPlug.state)
    hout = specificEnthalpy(outPlug.state)
