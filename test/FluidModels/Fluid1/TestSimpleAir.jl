...
"""
   module Fluid1_TestSimpleAir.jl
...
ModiaMath.plot(result, ["pipe.m_flow", "pipe.outPlug.p", "pipe.outPlug.r", "pipe.outPlug.T", "pipe.outPlug.d", "pipe.eta"], heading="Fluid1_TestSimpleAir.jl: TestLine", figure=1)
...
ModiaMath.plot(result, [("junction.inPlugA.m_flow", "junction.inPlugB.m_flow" , "junction.outPlugC.m_flow"),
                        ...
                        ("pipe1.outPlug.p", "pipe2.outPlug.p")], heading="Fluid1_TestSimpleAir.jl: TestJunction", figure=2)
...
result = simulate(TestLine2, 1.0, logTranslation=true, logSimulation=true, tearing=false, aliasElimination=false)
...
ModiaMath.plot(result, [("volume.inPlug.m_flow", "volume.outPlug.m_flow"),
                         "volume.medium.p",
                         "volume.medium.T",          # "fixedSource.outPlug.T"
                        ("volume.h", "volume.medium.h")], 
                         heading="Fluid1_TestSimpleAir.jl: TestLine2", figure=3)
...
@model TestLine3 begin
    ...
    volume2     = ClosedVolume(V=0.5, Medium=MediumUsedInExamples, 
                               medium = BaseProperties(MediumUsedInExamples, 
                                                       p_start=StaticPressure(start=1e5, state=false), 
                                                       T_start=Temperature(start=500.0, state=false)))
...
ModiaMath.plot(result, [("volume1.inPlug.m_flow", "volume1.outPlug.m_flow"),
                         "volume1.medium.p",
                        ("fixedSource.outPlug.state.T", "volume1.medium.T"),
                        ("volume1.h", "volume1.medium.h")], 
                         heading="Fluid1_TestSimpleAir.jl: TestLine3", figure=4)
=#
...
result = simulate(TestCompressor, 0.02; logTranslation=true, logSimulation=true, tearing=false, aliasElimination=false)
ModiaMath.plot(result, [ "compressor.inPlug.m_flow",
                        ("compressor.inPlug.p", "compressor.outPlug.p", "pipe2.outPlug.p")], 
                         heading="Fluid1_TestSimpleAir.jl: TestCompressor", figure=5)
...
result = simulate(TestLoop, 0.02; logTranslation=true, logSimulation=true, tearing=true, removeSingularities=true, aliasElimination=true)
ModiaMath.plot(result, [ "compressor.inPlug.m_flow",
                        ("compressor.inPlug.p", "compressor.outPlug.p", "pipe2.outPlug.p", "volume.medium.p")], 
                         heading="Fluid1_TestSimpleAir.jl: TestLoop", figure=6)
