...
mutable struct IdealMoistAirMediumData
    ...
    function IdealMoistAirMediumData(;cp_const=nothing, R_gas=nothing, MM_const=nothing, 
...
struct IdealMoistAirMedium <: CondensingGases
    infos::FluidInfos
    fluidConstants::SVector{2,BasicFluidConstants}
    fluidLimits::FluidLimits
    data::SVector{2, SimpleIdealGasMediumData}
    ...
    function IdealMoistAirMedium(; mediumName="MoistAir",
                                    ...
                                    T_default=293.15,
                                    X_default=[0.01, 0.99],
                                    fluidConstants=[nothing, nothing], 
        ...
        infos = FluidInfos(mediumName           = mediumName,
                           ...
                           baseProperties       = :BaseProperties_IdealMoistAirMedium,
                           singleState          = false,
                           reducedX             = true,
                           fixedX               = false,
                           ...
                           T_default            = T_default,
                           X_default            = X_default,
                           h_default            = specificEnthalpy(data, ThermodynamicState_pTX(p_default, T_default, X_default)))

        fluidLimits = FluidLimits(TMIN = data.T_min, 
                                  TMAX = data.T_max,
                                  HMIN = specificEnthalpy(data, ThermodynamicState_pTX(p_default, data.T_min, X_default)),
                                  HMAX = specificEnthalpy(data, ThermodynamicState_pTX(p_default, data.T_max, X_default)))
...
specificEnthalpy(data::SVector{2,IdealMoistAirMediumData}, state::ThermodynamicState_pTX)::Float64 = data.cp_const*(state.T - data.T0)
